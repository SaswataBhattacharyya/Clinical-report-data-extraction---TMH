...
def convert_long_to_dr_minit_format(df_long, order_file_path=None, output_path=None):
    """
    Convert long format DataFrame to Dr. Minit's wide format.
    
    Args:
        df_long: Long format DataFrame from the app
        order_file_path: Optional path to Excel file with column order
        output_path: Output folder path for saving the formatted Excel
    
    Returns:
        DataFrame in Dr. Minit's format
    """
    
    # If no order file provided, use default schema
    if order_file_path and os.path.isfile(order_file_path):
        desired_order = load_header_order(order_file_path)
    else:
        # Build default schema
        # Detect clinician columns (from "Serial" to the end)
        cols_lower = [str(c).strip().lower() for c in df_long.columns]
        if "serial" in cols_lower:
            serial_idx = cols_lower.index("serial")
            CLINICIAN_COLUMNS = list(df_long.columns[serial_idx:])
        else:
            CLINICIAN_COLUMNS = list(df_long.columns)

        # Build final schema (clinician + molbio)
        REQUIRED_ADMIN = [
        ...
        desired_order = FINAL_COLUMNS

    # Group by MRN and Date-Case_discussed
    grouped = df_long.groupby(["MRN", "Date-Case_discussed"], sort=False, dropna=False)
    ...
    for sheet, rows in sheets.items():
        if rows:  # Only add if there are rows
    ...
    # Combine all sheets
    if all_rows:
        # Ensure no duplicate columns before concatenating
        for i, df_sheet in enumerate(all_rows):
        ...
        # Create empty DataFrame with desired columns if no data
        final_df = pd.DataFrame(columns=desired_order)
    
    # Save to output path if provided
    if output_path:
        os.makedirs(output_path, exist_ok=True)
        output_file = os.path.join(output_path, "dr_minit_format.xlsx")
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for sheet, rows in sheets.items():
                if rows:
                    ...
                    # Create empty sheet
                    empty_df = pd.DataFrame(columns=desired_order)
                    empty_df.to_excel(writer, sheet_name=sheet, index=False)
        
        print(f"✅ Dr. Minit format saved: {output_file}")
