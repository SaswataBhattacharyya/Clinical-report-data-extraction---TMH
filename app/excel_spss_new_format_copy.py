...
# -------------------------------------------
def merge_sb_format(molbio_dir: str, clinician_dir: str, order_path: Optional[str] = None, out_dir: Optional[str] = None) -> pd.DataFrame:
    """
    Streamlit-compatible wrapper for the merge process.
    
    Args:
        molbio_dir: Folder path containing mol-bio Excel files
        clinician_dir: Folder path containing clinician Excel files  
        order_path: Optional Excel path for column order
        out_dir: Optional output folder path (if None, doesn't write file)
    ...
    for d in common_dates:
        ...
        merged_pair = merge_long_format(
            mdf, cdf,
            verbose=True,
            apply_blanking=False,
            apply_filldown=False
        )
