...
def bin_percentage(val):
    """Bin percentage values: <10%, 10-20%, ..., 90-100%"""
    try:
        v = float(val)
        if pd.isna(v):
            return None
        # Round to nearest 10
        rounded = int(round(v / 10) * 10)
        if rounded < 10:
            return "<10%"
        elif rounded >= 100:
            return "90-100%"
...
def bin_by_mean_std(series):
    """Bin by mean ± standard deviation, rounded to nearest 10"""
    mean = series.mean()
    std = series.std()
    ...
    def categorize(val):
        try:
            ...
            lower_bound = int(round((mean - std) / 10) * 10)
            mean_rounded = int(round(mean / 10) * 10)
            upper_bound = int(round((mean + std) / 10) * 10)
            
            if v < lower_bound:
                return f"< {lower_bound}"
            elif v < mean_rounded:
                return f"{lower_bound} to {mean_rounded}"
            elif v < upper_bound:
                return f"{mean_rounded} to {upper_bound}"
            else:
...
def create_month_columns(df):
    """Create Month columns for all date columns"""
    df = df.copy()
    date_cols = [col for col in df.columns if 'date' in col.lower()]
    
    for col in date_cols:
        try:
            df[f"Month_{col}"] = pd.to_datetime(df[col], errors='coerce').dt.strftime('%b %Y')
        except:
...
def create_binned_columns(df):
    """Create binned versions of continuous columns"""
    df = df.copy()
    
    # Find continuous columns (numeric, not in exclude list)
    exclude_cols = ['Serial', 'MRN', 'Age', 'Exon_no', 'Tier']
    
    for col in df.columns:
        if col in exclude_cols or df[col].dtype not in ['float64', 'int64']:
            continue
        
        col_lower = col.lower()
        # Check if it's a continuous column we want to bin
        if any(kw in col_lower for kw in ['vaf', 'depth', 'copy', 'score', 'fraction', 'read']):
            if 'vaf' in col_lower or '%' in col_lower:
                # Percentage-based binning
                df[f"{col}_Binned"] = df[col].apply(bin_percentage)
            else:
                # Mean±SD binning
                df[f"{col}_Binned"] = bin_by_mean_std(df[col])
    
    # Add Age binning if Age exists
        ...
        def create_age_bin(age):
            try:
                ...
                if age_val < 10:
                    ...
                    lower = int(age_val // 10) * 10
                    upper = lower + 10
                    return f"{lower}-{upper}"
            except (ValueError, TypeError):
                return None
        
        df["Age_Binned"] = df["Age"].apply(create_age_bin)
    
    return df

def build_combinations(x_axis_selections):
    """
    Build cross-product combinations from X-axis selections
    x_axis_selections = {'Gender': ['Male', 'Female'], 'Specimen': ['Tissue']}
    Returns: List of combination dicts
    """
    # Filter out columns with no checkboxes selected
    active_selections = {col: vals for col, vals in x_axis_selections.items() if vals}
    
    if not active_selections:
        return []
    
    # Create cross-product
    keys = list(active_selections.keys())
    values = list(active_selections.values())
...
def aggregate_data(df, combinations, y_metric):
    """
    For each combination, filter data and calculate Y metric
    Returns: DataFrame with [Label, Value] columns
    """
    results = []
    ...
    for combo in combinations:
        ...
        # Calculate Y metric
        if y_metric == "Count (unique MRN)":
            y_val = filtered[MRN_COL].nunique() if MRN_COL in filtered.columns else 0
        elif y_metric == "Count (total rows)":
            y_val = len(filtered)
        elif y_metric.startswith("Mean of "):
            col_name = y_metric.replace("Mean of ", "")
            y_val = filtered[col_name].mean() if col_name in filtered.columns else 0
        elif y_metric.startswith("Median of "):
            col_name = y_metric.replace("Median of ", "")
            y_val = filtered[col_name].median() if col_name in filtered.columns else 0
        else:
            y_val = 0
        ...
        # Create label
        label = " - ".join([f"{val}" for val in combo.values()])
        
        results.append({'Label': label, 'Value': y_val})
...
if not data_available or final_df is None:
    st.info("💡 Complete the merge above to activate graphical representation")
else:
    st.markdown("Visualize your filtered data with interactive charts")
    
    # Prepare data for graphing - add binned and month columns
    df_for_graph = final_df.copy()
    df_for_graph = create_month_columns(df_for_graph)
    df_for_graph = create_binned_columns(df_for_graph)
    
    # Graph Type Selection
    graph_type = st.radio(
        "Select Graph Type",
        ["Bar Chart", "Pie Chart", "Stacked Bar Chart", "Histogram"],
        horizontal=True,
        help="Choose the type of visualization"
    )
    
    # Two columns for X and Y axis configuration
    col_x, col_y = st.columns([2, 1])
    
    with col_x:
        st.markdown("**X-Axis Configuration**")
        
        # Get available categorical columns for X-axis
        exclude_from_x = ['MRN', 'Serial', 'Name', 'Data_Entry_Done_By', 'Source_File']
        
        # Categorical columns + Binned columns + Month columns + Gene_name and dependent columns
        x_axis_options = []
        for col in df_for_graph.columns:
            col_lower = col.lower()
            # Skip excluded columns
            if col in exclude_from_x:
                continue
            # Skip raw date columns (we have Month versions)
            if 'date' in col_lower and not col.startswith('Month_'):
                continue
            # Skip raw continuous columns (we have binned versions)
            if col.endswith('_Binned') or col.startswith('Month_') or df_for_graph[col].dtype == 'object':
                # Include binned, month, and categorical columns
                if col not in exclude_from_x and col != 'Age':  # Age is covered by Age_Binned
                    x_axis_options.append(col)
            elif df_for_graph[col].dtype in ['float64', 'int64']:
                # Check if this continuous column has a binned version
                if f"{col}_Binned" not in df_for_graph.columns and col not in ['Age', 'Serial', 'MRN']:
                    # This is a categorical numeric column, include it (Tier, Exon_no are now included)
                    x_axis_options.append(col)
        
        x_axis_options = sorted(list(set(x_axis_options)))
        
        # X-axis column selection
        selected_x_cols = st.multiselect(
            "Select X-Axis Columns",
            x_axis_options,
            default=[],
            help="Select one or more columns for X-axis"
        )
        
        # Display checkboxes for selected columns in horizontal scrollable area
        if selected_x_cols:
            st.markdown("**Select Values for Each Column:**")
            
            # Create horizontal container with columns
            num_cols = len(selected_x_cols)
            if num_cols > 0:
                # Use columns for horizontal layout
                checkbox_cols = st.columns(min(num_cols, 4))  # Max 4 columns at a time
                
                x_axis_selections = {}
                
                for idx, col in enumerate(selected_x_cols):
                    col_idx = idx % 4  # Wrap around if more than 4
                    
                    with checkbox_cols[col_idx]:
                        st.markdown(f"**{col}**")
                        
                        # Get unique values for this column
                        unique_vals = sorted(df_for_graph[col].dropna().astype(str).unique().tolist())
                        
                        # Create checkboxes
                        selected_vals = []
                        for val in unique_vals:
                            if st.checkbox(val, key=f"x_{col}_{val}"):
                                selected_vals.append(val)
                        
                        x_axis_selections[col] = selected_vals
        else:
            x_axis_selections = {}
    
    with col_y:
        st.markdown("**Y-Axis Metric**")
        
        # Build Y-axis options
        y_axis_options = ["Count (unique MRN)", "Count (total rows)"]
        
        # Add continuous columns for Mean/Median
        continuous_cols_for_y = []
        for col in df_for_graph.columns:
            if df_for_graph[col].dtype in ['float64', 'int64']:
                # Check if it's a real continuous column (not binned version)
                if not col.endswith('_Binned') and col not in ['Serial', 'MRN', 'Exon_no', 'Tier']:
                    continuous_cols_for_y.append(col)
        
        # Add Mean and Median options for continuous columns
        for col in sorted(continuous_cols_for_y):
            # Check if this column's binned version is selected in X-axis
            is_disabled = f"{col}_Binned" in selected_x_cols if selected_x_cols else False
            if not is_disabled:
                y_axis_options.append(f"Mean of {col}")
                y_axis_options.append(f"Median of {col}")
        ...
        y_metric = st.selectbox(
            ...
            y_axis_options,
            index=0,
            help="Choose what to measure on Y-axis"
        )
    ...
    if not selected_x_cols:
        st.warning("⚠️ Please select at least one X-axis column to generate the graph")
    elif not any(x_axis_selections.values()):
        st.warning("⚠️ Please select at least one value from the checkboxes")
    else:
        # Build combinations
        combinations = build_combinations(x_axis_selections)
        
        if not combinations:
            st.warning("⚠️ No valid combinations found")
        else:
            # Aggregate data
            agg_df = aggregate_data(df_for_graph, combinations, y_metric)
                ...
                try:
                    if graph_type == "Bar Chart":
                        fig = px.bar(
                            ...
                            y='Value',
                            title=f"{y_metric}",
                            labels={'Label': ', '.join(selected_x_cols), 'Value': y_metric}
                        )
                        fig.update_layout(
                            xaxis_title=', '.join(selected_x_cols),
                            yaxis_title=y_metric,
                            showlegend=True
                        )
                    
                    elif graph_type == "Pie Chart":
                        if len(selected_x_cols) > 1:
                            st.info("💡 Pie chart works best with single column. Showing combined categories.")
                        
                        fig = px.pie(
                            agg_df,
                            names='Label',
                            values='Value',
                            title=f"{y_metric} Distribution"
                        )
                    
                    elif graph_type == "Stacked Bar Chart":
                        # For stacked bar, we need to split the label
                        if len(selected_x_cols) >= 2:
                            # Split labels and create grouped data
                            agg_df['Primary'] = agg_df['Label'].apply(lambda x: x.split(' - ')[0])
                            agg_df['Secondary'] = agg_df['Label'].apply(lambda x: ' - '.join(x.split(' - ')[1:]) if len(x.split(' - ')) > 1 else '')
                            ...
                            fig = px.bar(
                                ...
                                color='Secondary',
                                title=f"{y_metric}",
                                labels={'Primary': selected_x_cols[0], 'Value': y_metric, 'Secondary': ' - '.join(selected_x_cols[1:])}
                            )
                            fig.update_layout(
                                xaxis_title=selected_x_cols[0],
                                yaxis_title=y_metric,
                                showlegend=True
                            )
                        else:
                    ...
                    # Show the aggregated data table
                    with st.expander("📋 View Aggregated Data"):
                        st.dataframe(agg_df, use_container_width=True)
                    
                except Exception as e:
                    st.error(f"❌ Error generating graph: {str(e)}")
                    st.info("Please try a different graph type or column selection")
