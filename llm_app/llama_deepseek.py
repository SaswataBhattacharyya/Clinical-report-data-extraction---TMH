...
# ---------- Config ----------
# Model will be set by main function (deepseek-r1:latest or llama3:latest)
model_name = None

def init_model(model="deepseek-r1:latest"):
...
# build_structured_prompt now accepts BOTH texts (grid + spaced) and includes the expanded schema
def build_structured_prompt(grid_text, spaced_text, discussion_date):
...
# regex to group the two collated inputs per report
PAIR_RE = re.compile(r"^(.*)_allign?ed_(spaced|grid)\.txt$", re.IGNORECASE)  # accepts 'alligned' or 'aligned'

def main(input_dir=None, output_dir=None, model="deepseek-r1:latest"):
    ...
    if output_dir is None:
        output_dir = input("Enter OUTPUT root folder (LLM outputs): ").strip()
    ...
    try:
        for root, _, files in os.walk(INPUT_DIR):
            ...
            # --- gather list docs (black/red/blue) & paired report files ---
            list_records = []
            list_xlsx_path = os.path.join(output_subdir, "list_output.xlsx")

            # group aligned pairs by base
            pairs = {}  # base -> {"spaced": path?, "grid": path?}
            ...
            for file in files:
                ...
                if any(k in low for k in ["black_list", "red_list", "blue_list"]):
                    ...
                    # attach color & filename for ordering
                ...
                # otherwise, try to match paired aligned inputs
            ...
            # --- process each report pair once (two inputs -> one LLM output) ---
            ...
            # --- write list output as Excel, ordered Black -> Red -> Blue, preserving in-file order ---
            if list_records:
        ...
        print("\n🎉 All folders processed. LLM outputs written; list Excel files saved.")
        return {"success": True, "message": f"Successfully processed LLM parsing from {INPUT_DIR}"}
    except Exception as e:
        return {"success": False, "message": f"Error during LLM parsing: {str(e)}"}
